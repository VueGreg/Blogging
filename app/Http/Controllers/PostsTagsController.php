<?php

namespace App\Http\Controllers;

use App\Models\PostsTags;
use App\Http\Requests\StorePostsTagsRequest;
use App\Http\Requests\UpdatePostsTagsRequest;

class PostsTagsController extends Controller
{
    /**
     * Display a listing of the resource.
     */
    public function index()
    {
        //
    }

    /**
     * Show the form for creating a new resource.
     */
    public function create()
    {
        //
    }

    /**
     * Store a newly created resource in storage.
     */
    public function store(StorePostsTagsRequest $request)
    {
        //
    }

    /**
     * Display the specified resource.
     */
    public function show(PostsTags $postsTags)
    {
        //
    }

    /**
     * Show the form for editing the specified resource.
     */
    public function edit(PostsTags $postsTags)
    {
        //
    }

    /**
     * Update the specified resource in storage.
     */
    public function update(UpdatePostsTagsRequest $request, PostsTags $postsTags)
    {
        //
    }

    /**
     * Remove the specified resource from storage.
     */
    public function destroy(PostsTags $postsTags)
    {
        //
    }
}
